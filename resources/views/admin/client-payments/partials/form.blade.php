...
<div class="mb-3">
    ...
    <input type="text" class="form-control" id="client_id"
        aria-describedby="Client Id" name="client_id"
        value="@isset($client) {{ $client->id }} @endisset" @isset($edit) disabled @endisset>
    @error('client_id')
...
<div class="mb-3">
    ...
    <input type="text" class="form-control @error('client_name') is-invalid @enderror" id="client_name"
        ...
        value="@isset($client) {{ $client->company }} @endisset"  @isset($edit) disabled @endisset>
    @error('client_name')
        <span class="invalid-feedback" role="alert">
            {{ $message }}
        </span>
...
<div class="mb-3">
    ...
    <input type="text" class="form-control @error('payment_terms') is-invalid @enderror" id=""
        aria-describedby="Payment Terms" name="payment_terms" id="payment_terms" value="{{ old('payment_terms') }}@isset($client_payment_profile) {{ $client_payment_profile->payment_terms }} @endisset">
    @error('payment_terms')
        <span class="invalid-feedback" role="alert">
            {{ $message }}
        </span>
...
<div class="mb-3">
    ...
    <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" aria-describedby="email"
        name="email" value="@isset($client) {{ $client->email }} @endisset" @isset($edit) disabled @endisset>
    @error('email')
        <span class="invalid-feedback" role="alert">
            {{ $message }}
        </span>
...
<div class="mb-3">
    ...
    <textarea class="form-control" aria-label="With textarea" rows="7" cols="50" name="email_message"
        id="email_message" @isset($edit) disabled @endisset></textarea>
</div>
